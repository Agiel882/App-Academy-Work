...
class Play
  ...
  def self.find_by_playwright(name)
    data = PlayDBConnection.instance.execute("SELECT plays.id, title, year, playwright_id FROM plays JOIN playwrights ON plays.playwright_id = playwrights.id WHERE name = '#{name}'")
    data.map {|datum| Play.new(datum)}
  end
