...
def pulp_fiction_actors
  ...
  Movie.joins(:actors).select("actors.id, actors.name").where(title: "Pulp Fiction")
end
...
def uma_movies
  ...
  Actor.joins(:movies).select("movies.id, movies.title, movies.yr").where(name: "Uma Thurman").order("movies.yr")
end
